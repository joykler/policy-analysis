...
# Topics to curate
remaining_topics = ['Doorwerking_Continuiteit', 'Erkenning_Verantwoordelijkheid', 'Kennis_Herinnering']

for topic_name in remaining_topics:
    print("\n\n" + "="*80)
    print(f"{topic_name.upper().replace('_', ' ')} - CURATION")
    print("="*80)

    # Filter for topic
    df = df_all[df_all['topic'] == topic_name].copy()
    ...
    if len(suspicious) > 0:
        print(suspicious[['term', 'category', 'weight', 'cosine', 'df', 'parent']].head(15).to_string(index=False))
    ...
    for idx, row in df[ultra_high].iterrows():
        ...
        # Generic terms - move to CONTEXT
        if term in ['mensen', 'personen', 'groepen', 'Nederland', 'Nederlandse', 'maatschappij',
                    'samenleving', 'geschiedenis', 'cultuur', 'overheid']:
    ...
    # RULE 2: Low cosine + BELEID/KERN - validate topic specificity
    ...
    # Save results
    topic_file = topic_name.lower()
    output_file = fr'C:\Users\Home\policy-analysis\{topic_file}_curated.csv'
    df.to_csv(output_file, index=False, encoding='utf-8-sig')

    final_output = fr'C:\Users\Home\policy-analysis\{topic_file}_final.csv'
    ...
    if (df['action'] == 'REMOVE').sum() > 0:
        ...
        print(removal_reasons.head(10))
        print(f"\nTotal removed: {len(removed)}")

print("\n\n" + "="*80)
print("ALL REMAINING TOPICS CURATED")
print("="*80)
