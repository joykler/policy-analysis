...
SOURCE_FOLDER   = config["SOURCE_FOLDER_PATH"]
...
def is_not_english(sentence, min_words=4):
...
def clean_and_tokenize(text, stop_words, stemmer):
    # Remove non-letter/number, lowercase, remove stopwords, stem, remove short
...
def process_file_chunk(
    ...
    try:
        ...
        # Split into sentences, filter out English, re-join
        sentences = re.split(r'(?<=[.!?])\s+', raw_text)
        filtered_sentences = [s for s in sentences if is_not_english(s)]
        filtered_text = ' '.join(filtered_sentences)

        # Tokenize using spacy for good Dutch handling
        tokens = split_tokens_spacy(filtered_text, nlp)
        # Clean & stem tokens, remove stopwords, short words
        tokens = [stemmer.stem(w) for w in tokens if w.lower() not in stop_words and len(w) > 2 and w.isalpha()]
        # Chunk by token count, assign chunk number
        chunks = chunk_tokens(tokens, tokens_per_chunk)

        for i, chunk_tokenlist in enumerate(chunks):
            if not chunk_tokenlist:
                # Save empty chunk as well
                out_list.append({
                    'filename': fname,
                    'filepath': file_path,
            ...
            # Compute also "classic" cosine with anchors (highest across all topics)
            cosine = compute_cosine_with_anchors(chunk_tokenlist, topic_word_lists, vectorizer)

            # Assign topic as true **only if**:
            # (a) assigned by Corex **AND**
            # (b) cosine_with_anchors OR weighted_cosine > threshold
            topic_label_mask = []
            ...
            out_list.append({
                ...
                'cosine': cosine,
                'cosine_weighted': topic_cosines_weighted,
...
if __name__ == "__main__":
    ...
    if USE_FOLDER:
        files = find_txt_files_by_folder(SOURCE_FOLDER, year_filter=YEAR_FILTER, doc_types=DOC_TYPES, simple_mode=FOLDER_MODE)
