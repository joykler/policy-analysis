...
# Filter for Koninkrijks_Macht
df = df_all[df_all['topic'] == 'Koninkrijks_Macht'].copy()

print("="*80)
print("KONINKRIJKS_MACHT - CURATION")
print("="*80)
...
# RULE 1: Remove semantic drift - common patterns
# For Koninkrijks_Macht, watch for: general governance terms, non-Kingdom admin terms
...
for idx, row in df[ultra_high].iterrows():
    ...
    if term in ['Nederland', 'Nederlandse', 'overheid', 'regering']:
...
# RULE 3: Low cosine + BELEID/KERN - validate policy-specificity
low_cos_policy = (df['cosine'] < 0.72) & (df['category'].isin(['BELEID', 'KERN'])) & (df['action'] == 'KEEP')

# Generic government terms that aren't Kingdom-specific
generic_govt = ['bestuur', 'beheer', 'administratie', 'bureaucratie', 'ambtelijk']

for idx, row in df[low_cos_policy].iterrows():
    term = row['term']

    # Check if term is Kingdom-specific or just generic governance
    if any(gen in term for gen in generic_govt) and 'Koninkrijk' not in term and 'Rijk' not in term:
        # Likely too generic
        df.loc[idx, 'action'] = 'REMOVE'
        df.loc[idx, 'curation_notes'] = f'Generic governance term, not Kingdom-specific (cosine {row["cosine"]:.3f})'
...
# RULE 4: CONTEXT category validation - should be geographic/constitutional markers
context_terms = df[df['category'] == 'CONTEXT']
print(f"\nCONTEXT terms: {len(context_terms)}")
# These should already be appropriate (seeds were curated)
...
# Save results
output_file = r'C:\Users\Home\policy-analysis\koninkrijks_macht_curated.csv'
df.to_csv(output_file, index=False, encoding='utf-8-sig')

final_output = r'C:\Users\Home\policy-analysis\koninkrijks_macht_final.csv'
