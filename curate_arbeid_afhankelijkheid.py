...
# Filter for Arbeid_Afhankelijkheid
df = df_all[df_all['topic'] == 'Arbeid_Afhankelijkheid'].copy()

print("="*80)
print("ARBEID_AFHANKELIJKHEID - CURATION")
print("="*80)
...
# RULE 1: Ultra-high frequency dampening (df > 300)
ultra_high = (df['df'] > 300) & (df['action'] == 'KEEP')
for idx, row in df[ultra_high].iterrows():
    term = row['term']
    curr_weight = row['weight']

    # Move very generic labor terms to CONTEXT
    if term in ['werk', 'arbeid', 'arbeiders', 'werknemers', 'lonen', 'economie']:
...
# RULE 2: Low cosine + BELEID/KERN - validate labor exploitation specificity
low_cos_policy = (df['cosine'] < 0.72) & (df['category'].isin(['BELEID', 'KERN'])) & (df['action'] == 'KEEP')

# Generic labor/economic terms that aren't exploitation-specific
generic_labor = ['werk', 'baan', 'inkomen', 'salaris', 'economisch', 'markt']

for idx, row in df[low_cos_policy].iterrows():
    term = row['term']

    # Check if term is exploitation-specific or just generic labor
    if any(gen in term for gen in generic_labor) and 'dwang' not in term and 'uitbuiting' not in term and 'ongelijk' not in term:
        df.loc[idx, 'action'] = 'REMOVE'
        df.loc[idx, 'curation_notes'] = f'Generic labor/economic term, not exploitation-specific (cosine {row["cosine"]:.3f})'
...
# RULE 3: Over-expanded parents (>30) - apply stricter review
...
# Save results
output_file = r'C:\Users\Home\policy-analysis\arbeid_afhankelijkheid_curated.csv'
df.to_csv(output_file, index=False, encoding='utf-8-sig')

final_output = r'C:\Users\Home\policy-analysis\arbeid_afhankelijkheid_final.csv'
