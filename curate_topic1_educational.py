"""
Systematic Curation of Topic 1: Educational Disadvantage & Brain Drain
...
# Filter for Topic 1
topic1 = df[df['topic'] == 'Educational Disadvantage & Brain Drain'].copy()

print(f"Total terms for Educational Disadvantage & Brain Drain: {len(topic1)}")
print(f"\nWeight distribution:")
print(topic1['weight'].value_counts().sort_index(ascending=False))

print(f"\nCategory distribution:")
print(topic1['category'].value_counts())

print(f"\nDocument frequency statistics:")
print(topic1['df'].describe())

print(f"\nCosine similarity statistics:")
print(topic1['cosine'].describe())

# Add curation decision column
topic1['curation_decision'] = 'REVIEW'
topic1['curation_reason'] = ''

print("\n" + "="*80)
print("STARTING SYSTEMATIC CURATION - TOPIC 1")
print("="*80)
...
fragments = topic1[(topic1['term'].str.len() < 4) & (topic1['is_seed'] == 0)]
print(f"Morphological fragments (len<4, not seed): {len(fragments)}")
if len(fragments) > 0:
    print(fragments[['term', 'parent', 'cosine', 'df', 'weight']].to_string())
    topic1.loc[fragments.index, 'curation_decision'] = 'REMOVE'
    topic1.loc[fragments.index, 'curation_reason'] = 'Morphological fragment'

# 2. Extreme low similarity (cosine < 0.65)
low_cosine = topic1[(topic1['cosine'] < 0.65) & (topic1['is_seed'] == 0)]
print(f"\nLow cosine (<0.65, not seed): {len(low_cosine)}")
if len(low_cosine) > 0:
    print(low_cosine[['term', 'parent', 'cosine', 'df', 'weight']].to_string())
    topic1.loc[low_cosine.index, 'curation_decision'] = 'REMOVE'
    topic1.loc[low_cosine.index, 'curation_reason'] = 'Extreme low similarity'

# 3. Single document frequency (df == 1)
single_df = topic1[(topic1['df'] == 1) & (topic1['is_seed'] == 0)]
print(f"\nSingle document frequency (df=1, not seed): {len(single_df)}")
if len(single_df) > 0:
    print(single_df[['term', 'parent', 'cosine', 'df', 'weight']].head(20).to_string())
    topic1.loc[single_df.index, 'curation_decision'] = 'REMOVE'
    topic1.loc[single_df.index, 'curation_reason'] = 'Single document frequency'

# Summary of automatic removals
auto_removed = topic1[topic1['curation_decision'] == 'REMOVE']
print(f"\n### PHASE 1 SUMMARY ###")
print(f"Total automatic removals: {len(auto_removed)}")
print(f"Remaining for manual review: {len(topic1) - len(auto_removed)}")
...
manual_review = topic1[(topic1['curation_decision'] == 'REVIEW') & (topic1['is_seed'] == 0)].copy()
...
output_file = r"C:\Users\Home\policy-analysis\topic1_educational_manual_review.csv"
...
# Save current state
topic1.to_csv(r"C:\Users\Home\policy-analysis\topic1_educational_curation_state.csv", index=False)
print(f"\n\nCuration state saved to: topic1_educational_curation_state.csv")
